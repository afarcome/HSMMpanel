...
sigma=exp(theta[k-1+k*2+k*p+q*p+1:p])
clusterExport(cl,c("Y","X","P","likn","xi","d","be","sigma", "init"),envir=environment())
-sum(parSapply(cl,1:n,function(i) likn(init,d,xi,be,sigma,P,Y[i,,],X[i,,])))}

estHSMMpanelCont=function(Y,X,k,cl,init, Pinit, se=FALSE) {
...
sigma=exp(thetaInit[5+2*p+q*q+1:p])}
...
sigma=exp(thetaInit[k-1+k*2+k*p+q*p+1:p])
}

score=ses=info=NA 

if(se==TRUE) {
clusterExport(cl,c("forwn","gradn","dpar","hessn","Y","X","P","likn","xi","d","be","sigma","init"),envir=environment())
alphas=parSapply(cl,1:n,function(i) forwn(init,d,xi,be,sigma,P,Y[i,,],X[i,,]),simplify=FALSE)
clusterExport(cl,c("forwn","gradn","dpar","hessn","Y","X","P","likn","xi","d","be","init","sigma","alphas"),envir=environment())
grn=parSapply(cl,1:n,function(i) gradn(init,dpar,xi,be,sigma,P,Y[i,,],X[i,,],alphas[[i]]),simplify=FALSE)
clusterExport(cl,c("sigma","forwn","gradn","dpar","hessn","Y","X","P","likn","xi","d","be","init","alphas","grn"),envir=environment())
hs=parSapply(cl,1:n,function(i) hessn(init,dpar,xi,be,sigma,P,Y[i,,],X[i,,],alphas[[i]],grn[[i]]),simplify=FALSE)
...
likn=function(init,d,xi,be,sigma,P,x,cov) {
library(snipEM)
p=ncol(x)
lx=nrow(x)
k=ncol(d)
...
probs[1,1,]=log(init)+apply(dnorm(x[1,],mu,sigma,log=T),2,sum)
...
apply(dnorm(x[j,],mu,sigma,log=T),2,sum)})
...
forwn=function(init,d,xi,be,sigma,P,x,cov) {
library(snipEM)
p=ncol(x)
lx=nrow(x)
k=ncol(d)
...
gradn=function(init,dpar,xi,be,sigma,P,x,cov,alphas) {
...
exp(sum(dnorm(x[1,],mu,sigma,log=T)))
}
...
sk=exp(sum(dnorm(x[1,],mu,sigma,log=T)))

for(u in 1:(k-1)) {
mu=as.vector(be%*%cov[1,])+xi[,u]
gr=grad(f,c(xi,be),method="simple",u=u)
probs[1,1,u,u]=exp(sum(dnorm(x[1,],mu,sigma,log=T)))
...
{
mu=as.vector(be%*%cov[j,])+xi
exp(apply(dnorm(x[j,],mu,sigma,log=T),2,sum))})
...
exp(sum(dnorm(x[j,],mu,sigma,log=T)))}))}
...
hessn=function(init,dpar,xi,be,sigma,P,x,cov,alphas,grn) {
# discrete gamma dwelling
library(snipEM)
library(numDeriv)
p=ncol(x)
